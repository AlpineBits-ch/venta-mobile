...
class _MembersSettingsTabState extends State<MembersSettingsTab> {
  ...
  void _onSearchChanged(String value) {
    _debounce?.cancel();
    _debounce = Timer(const Duration(milliseconds: 300), () => _load(query: value.trim()));
  }

  Future<void> _kick(GuildMemberDto member) async {
    final displayName = member.nickname ?? member.profile?.userName ?? member.userId;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Could not kick that member.')));
      }
    }
  }
