...
void main() {
  group('MessagePushPayload', () {
    ...
    test('ignores anything that is not a message push', () {
      expect(
        MessagePushPayload.tryParse({'type': 'call', 'callId': 'c1'}),
        isNull,
      );
  ...
  group('MlsStore plaintext cache', () {
    ...
    test(
      'a reload never overwrites what this process just decrypted',
      () async {
        final app = open();
        await app.init('usr_1');
    ...
    group('at rest', () {
      ...
      test(
        'a sealed file with no key at all is left alone, not replaced',
        () async {
          final app = open();
          await app.init('usr_1');
      ...
      test(
        'an unsealed file written by an older build is sealed on load',
        () async {
          ...
          await file.writeAsString(
            jsonEncode({
              'msg_legacy': base64Encode(utf8.encode('from before')),
            }),
          );
          ...
          expect(
            ...
            isTrue,
            reason:
                'sealed on that load, not on some later incidental write - '
      ...
      test(
        'a launch with no key writes nothing rather than writing it bare',
        () async {
          final store = keyless();
      ...
      test(
        'clearing still clears when there is no key to rewrite with',
        () async {
    ...
    test(
      'plaintext does not leak across contexts on a reused message id',
      () async {
        final app = open();
        await app.init('usr_1');
        app.cacheMessage(
        ...
        expect(
          ...
          isNull,
          reason:
              'a message id the server picked must not address another '
    ...
    test(
      'prunes down to the cap, keeping what was read most recently',
      () async {
        final app = open();
        await app.init('usr_1');
        ...
        expect(
          read(app, 'msg_1'),
          isNull,
          reason: 'the least recently read goes',
        );
      },
    );
  });
